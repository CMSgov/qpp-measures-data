...
class Measure(StringFormatterMixin):
    ...
    def compare(self, other):
        self.diff_list = self.diff_list = self.append_string(f"# DIFF REPORT FOR MEASURE {self.id} (comparing {self.filename} and {other.filename} )", False)
        if not all(self.eligibility_options_eq):
            self.diff_list = self.diff_list = self.append_string("## Eligibility Options Details\n", False)
    ...
    def get_po_diff(self, other):
        ...
        self.diff_list = self.append_string(f"**There are {len(common_quality_codes)} common performance options for both {self.year} and {other.year}:**", add_tab=False)
        self.diff_list = self.append_string(f"\n{self.convert_list_to_bullets(common_quality_codes)}")
        ...
        if len(quality_codes_only_in_po1):
            self.diff_list = self.append_string(f"**There are {len(quality_codes_only_in_po1)} performance options in {self.year} but not {other.year}:** \n{self.convert_list_to_bullets(quality_codes_only_in_po1)}", add_tab=False)
        if (len(quality_codes_only_in_po2)):
            self.diff_list = self.append_string(f"**There are {len(quality_codes_only_in_po2)} performance options in {self.year} but not {other.year}:** \n{self.convert_list_to_bullets(quality_codes_only_in_po2)}", add_tab=False)
        ...
        if in_1_not_in_2:
            self.diff_list = self.append_string(f"**Performance options detail in {self.year} not in {other.year}:**", add_tab=False)
            for x in in_1_not_in_2:
                self.diff_list = self.append_string(str(x))
        if in_2_not_in_1:
            self.diff_list = self.append_string(f"**Performance options detail in {other.year} not in {self.year}:**", add_tab=False)
            for x in in_2_not_in_1:
                self.diff_list = self.append_string(str(x))
        
        common_po_pairs = [(po1, po2) for po1,po2 in zip(common1, common2)]
        self.diff_list = self.append_string("**Common Performance option pairs that differ in detail:**", add_tab=False)
        for po1, po2 in common_po_pairs:
            ...
            for attr in po1.__dict__.keys():
                if po1.__dict__[attr] != po2.__dict__[attr]:
                    should_print = True
            if should_print:
                self.diff_list = self.append_string(f"{self.year} Performance Option: {po1}")
                self.diff_list = self.append_string(f"{other.year} Performance Option: {po2}")
                self.diff_list = self.append_string("---", add_tab=False)
            ...
            for attr in eo1.__dict__.keys():
                if eo1.__dict__[attr] != eo2.__dict__[attr]:
                    if attr == "procedure_codes" or attr == "additional_procedure_codes":
                        key = "code"
                        self.get_missing_measures(eo1.__dict__[attr], eo2.__dict__[attr], key, attr, other.year)
                        common_measures = self.get_common_measures(eo1.__dict__[attr], eo2.__dict__[attr], key)
                        self.compare_common_pairs(common_measures, other.year, attr)
                    elif attr == "diagnosis_codes":
                        self.get_missing_measures_lists(eo1.__dict__[attr], eo2.__dict__[attr], attr, other.year)
                        self.append_string("\n")
                    elif attr in ["option_group", "min_age"]:
                        self.handle_simple_attribute(attr, other.year, eo1.__dict__[attr], eo2.__dict__[attr])
                    else:
                        raise NotImplementedError(f"You need to account for {attr}")



    def handle_simple_attribute(self, attr, other_year, eo1, eo2):
        self.diff_list = self.append_string(f"**Differing {attr} Pair:**", add_tab=False)
        self.diff_list = self.append_string(f"{self.year} eligibilityOptions.{attr} = {eo1}")
        self.diff_list = self.append_string(f"{other_year} eligibilityOptions.{attr} = {eo2}")
    ...
    def compare_common_pairs(self, common_measures, other_year, attr):
        for m1,m2 in common_measures:
            if m1 == m2:
                ...
                self.diff_list = self.append_string(f"**Differing {attr} Pair for code {m1.code}:**", add_tab=False)
                self.diff_list = self.append_string(f"{attr} from {self.year}: {m1}")
                self.diff_list = self.append_string(f"{attr} from {other_year}: {m2}")


    def get_missing_measures_lists(self, measures1, measures2, attr, other_year):
        in_1_not_2 = [m for m in measures1 if m not in measures2]
        if in_1_not_2: 
            self.diff_list = self.append_string(f"**{attr} in {self.year} but not {other_year}:**", add_tab=False)
            self.diff_list = self.append_string(", \n".join(in_1_not_2))
        in_2_not_1 = [m for m in measures2 if m not in measures1]
        if in_2_not_1: 
            self.diff_list = self.append_string(f"**{attr} in {other_year} but not {self.year}:**", add_tab=False)
            self.diff_list = self.append_string(", \n".join(in_2_not_1))

    def get_missing_measures(self, measures1, measures2, key, attr, other_year):
        in_1_not_2 = [m for m in measures1 if m.__dict__[key] not in [m.__dict__[key] for m in measures2]]
        if in_1_not_2: 
            self.diff_list = self.append_string(f"**{attr}.{key} in {self.year} but not {other_year}:**", add_tab=False)
            self.diff_list = self.append_string("".join([str(x) for x in in_1_not_2]))
        in_2_not_1 = [m for m in measures2 if m.__dict__[key] not in [m.__dict__[key] for m in measures1]]
        if in_2_not_1: 
            self.diff_list = self.append_string(f"**{attr}.{key} in {other_year} but not {self.year}:**", add_tab=False)
            self.diff_list = self.append_string("".join([str(x) for x in in_2_not_1]))
