...
class SingleSourceJsonComparisonRunner(StringFormatterMixin):
    ...
    def process_measure(self, id, code, filename):
        ...
        m.eligibility_options = self.dispatch("eligibilityOptions", code["eligibilityOptions"])
        m.measure_id = self.dispatch("measureId", code["measureId"])
        m.performance_options = self.dispatch("performanceOptions", code["performanceOptions"])
        return m
    ...
    def create_eligibility_option(self, opt):
            ...
            eo.additional_procedure_codes = sorted(self.process_additional_procedure_codes(opt)) if opt.get("additionalProcedureCodes") else []
            return eo
    ...
    def get_missing_measures(self, measures1, measures2, file1, file2):
        ...
        self.diff_list = self.append_string(f"**Measures in {file1} but not in {file2}:**\n {self.convert_list_to_bullets(in_1_not_2)}", False)
        in_2_not_1 = [m.id for m in measures2 if m.id not in [m.id for m in measures1]]
        self.diff_list = self.append_string(f"**Measures in {file2} but not in {file1}:**\n {self.convert_list_to_bullets(in_2_not_1)}", False)
    ...
    def get_common_measures(self, measures1, measures2):
        ...
        self.diff_list = self.append_string(f"**Measures in both that differ:**\n  {self.convert_list_to_bullets([m[0].id for m in common])}", False)
        return common
    ...
    def load_file_data(self):
        with open(f"{self.source_dir}/{self.file1}") as f1, open(f"{self.source_dir}/{self.file2}") as f2:
            data1 = json.load(f1)
            data2 = json.load(f2)
            return (data1, data2)
    ...
    def run(self):
        ...
        self.diff_list = self.append_string(f"# SUMMARY DIFF REPORT COMPARING {self.file1} and {self.file2}", False)
